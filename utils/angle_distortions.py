...
def angle_distortions(mesh_orig, mesh_param):
    ...
    with torch.no_grad():
        ...
        angle_orig = compute_per_vertice_min_angle(verts_packed_orig,faces_packed_orig)
        angle_param = compute_per_vertice_min_angle(verts_packed_param,faces_packed_param)
...
def compute_per_face_min_angle(V,F):
    ...
    # normailize
    u = u/torch.norm(u,dim=1).reshape([-1,1,3])
    v = v/torch.norm(v,dim=1).reshape([-1,1,3])
    
    # compute angles
    A = torch.acos(torch.sum(u*v, 1))
    A, _ = A.min(1)
    return A

def compute_per_vertice_min_angle(V,F):
    u = V[F[:,[1,2,0]],] - V[F[:,[0,1,2]]]
    v = V[F[:,[2,0,1]],] - V[F[:,[0,1,2]]]
    ...
    U = torch.sparse_coo_tensor(torch.vstack([m.repeat(3), F.flatten()]), A.flatten())
    U = U.to_dense()
    U[U == 0] = 10
    A, _ = torch.min(U, 0)
